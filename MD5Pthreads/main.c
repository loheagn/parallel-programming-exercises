...
void MD5Init(mdContext) MD5_CTX *mdContext;
{
...
void MD5Update(mdContext, inBuf, inLen) MD5_CTX *mdContext;
unsigned char *inBuf;
unsigned int inLen;
{
    UINT4 in[16];
...
void MD5Final(mdContext) MD5_CTX *mdContext;
{
    UINT4 in[16];
    int mdi;
    unsigned int i, ii;
...
void *worker(void *rangePtr) {
    struct Range *range = (struct Range *)rangePtr;
    for (int i = range->l; i < range->r; i++) {
        char *x = compute(i);
        char *md5 = MDString(x);
        if (!strcmp(md5, target)) {
            printf("%s\n", x);
            pthread_exit(0);
        }
        free(x);
...
char *argv[];
{
    target = malloc(33 * sizeof(char));
    scanf("%s", target);
    pthread_t *threadHandles;
    threadHandles = (pthread_t *)malloc(THREAD_COUNT * sizeof(pthread_t));

    int total = 36 * 36 * 36 * 36;
